...
class IndexedRDD(RDD):
    ...
    def updatable(rddObj, z=lambda K, U: V, f=lambda K, V, U: V):
        ...
        partitions = elemsPartitioned.mapPartitions(
            (lambda elementsIter: DictList(DictPartition(elementsIter))), True)
        return (partitions)
...
#------------------------ Get related functions --------------------------
    ...
    def get(self, key):
        ...
        results = self.indexedRDD.ctx.runJob(
            self, IndexedRDD.getElementsFromPartition(key), partitionIDs, True)
        return (results)
...
#------------------------ Put related functions --------------------------
  ...
  that reflects the modification."""
    ...
    def put(self, keyList):
        ...
        results = self.indexedRDD.mapPartitionsWithIndex(
            IndexedRDD.putElementsInPartition(partitionID, keyList), True)
    ...
    def putElementsInPartition(partitionID, keyList):
        def innerFunc(index, partElements):
            if(partitionID == index):
                dictPartitionObj = DictPartition(
                    partElements.dictPartitionList[0])
...
#------------------------ Delete related functions -----------------------
    ...
    def delete(self, key):
        ...
        results = self.indexedRDD.mapPartitionsWithIndex(
            IndexedRDD.delFromPartitionFunc(partitionID, key), True)
        return IndexedRDD(results)
...
#------------------------ Filter related functions -----------------------
    ...
    def mapIndexedRDDPartitions(self, f):
        newPartitionsRDD = self.indexedRDD.mapPartitions(
            lambda d: filter(f, d), True)
...
#------------------------ Join related functions -------------------------
    ...
    def join(self, other, f):
        ...
        newPartitionsRDD = resultRDD.mapPartitions(
            IndexedRDD.filterOnPartitionFunc(f), True)
...
def main():
    ...
    joinRDD = delRDD.leftJoin(
        rddZ, lambda id_a_b: (
            id_a_b[0], (id_a_b[1][0], id_a_b[1][1])))
